 ...
 * @author oscar
 */
public class p04_projecteFinal {
	...
	public static void main(String[] args) {
					...
					if (seguent >= 1) {
						...
						for (q = 0; q < comptador; q++) {
							if (places[q] >= seguent) {
								...
								System.out.println(id[q] + "\t" + places[q] + "\t" + res_ru + "\t" + tipus_hab + "\t"
										+ telefon[q]); // mostra només si el nombre és igual o superior al demanat
							}
						}
					}
				} else {
				...
				if (opcioCorrecta) {
					...
					for (q = 0; q < comptador; q++) {
						...
						if (tipus[q] == tipusHab) {
							System.out.println("\nID\tPlaces\tRus/Ucraïnès\tTipus\t\t\t\tTelèfon");
							System.out.println(
									id[q] + "\t" + places[q] + "\t" + res_ru + "\t" + tipus_hab + "\t" + telefon[q]);
						}
		...
		// PART C TONI; ORDENACIO ASCENDENT ARRAY;
		...
		int n = id.length;
		int temp = 0;
		System.out.println("Vols veure tot per ordre de ID?");
		if (limpito = scan.hasNextInt()) {
			maybe = scan.nextInt();
			limpito = true;
		} else {
			System.out.println("ok,seguim!");
			limpito = false;
			scan.nextLine();
		}
		if (limpito = false) {
			...
			for (int b = 0; b < comptador; b++) {
				...
				for (int c = 0; c < comptador - 1; c++) {
					...
					if (id[b] < id[c]) {
						...
						// tornem a ordenar tot;id, aquesta mana ara
						aux = id[b];
						id[b] = id[c];
						id[c] = aux;
						...
						// bombolla idioma (que a més intercanviem per l'string
						aux = parlaRusUcrania[b];
						parlaRusUcrania[b] = parlaRusUcrania[c];
						parlaRusUcrania[c] = aux;

						// bombolla tipus d'habitació (el mateix que passa amb idioma)
						aux = tipus[b];
						tipus[b] = tipus[c];
						tipus[c] = aux;
...
//ACABA ORDENACIO ASCENDENT TONI 
