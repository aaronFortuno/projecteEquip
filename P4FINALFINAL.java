 ...
 * @author oscar
 */
public class p04_projecteFinal2 {
	...
	public static void main(String[] args) {
		...
		if (opcioCorrecta) {
			...
			for (q = 0; q < comptador; q++) {
				...
				System.out.println(families[q][0] + "\t" + families[q][1] + "\t" + res_ru + "\t\t\t" + tipus_hab + "\t"
						+ families[q][4]);
			}
		}
					...
					if (seguent >= 1) {
						...
						for (q = 0; q < comptador; q++) {
							if (families[q][1] >= seguent) {
								...
								System.out.println(families[q][0] + "\t" + families[q][1] + "\t" + res_ru + "\t"
										+ tipus_hab + "\t" + families[q][4]); // mostra només si el nombre és igual o
																				// superior al demanat
							}
						}
					}
				...
				if (opcioCorrecta) {
					...
					for (q = 0; q < comptador; q++) {
						...
						if (families[q][3] == tipusHab) {
							...
							// bombolla descendent TONI
							for (int b = 0; b < comptador; b++) {

								for (int c = 0; c < comptador - 1; c++) {

									if (families[b][0] > families[c][0]) {

										// inter primera column id
										aux = families[b][0];
										families[b][0] = families[c][0];
										families[c][0] = aux;

										// inter 2a column places
										aux = families[b][1];
										families[b][1] = families[c][1];
										families[c][1] = aux;

										// inter column llengua
										aux = families[b][2];
										families[b][2] = families[c][2];
										families[c][2] = aux;
